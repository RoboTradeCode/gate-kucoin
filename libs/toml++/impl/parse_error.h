...
TOML_NAMESPACE_START
{
	...
	class parse_error TOML_PARSE_ERROR_BASE
	{
	  ...
	  public:
		...
		TOML_NODISCARD_CTOR
		parse_error(std::string&& desc, source_region&& config) noexcept //
			: description_{ std::move(desc) },
			  source_{ std::move(config) }
		{}

		TOML_NODISCARD_CTOR
		parse_error(std::string&& desc, const source_region& config) noexcept //
			: parse_error{ std::move(desc), source_region{ config } }
		{}
